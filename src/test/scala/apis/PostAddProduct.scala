...
class PostAddProduct extends Simulation {

  val httpConfig = http.baseUrl("https://fakestoreapi.com")
    .acceptHeader("application/json")
    .contentTypeHeader("application/json")

  val csvFeeder = csv("bodyRequest/bodyAddProduct.csv").circular

  // Escenario para el Modelo abierto
  val escenarioAbierto = scenario("Add Products - Modelo Abierto")
    .feed(csvFeeder)
    .repeat(1) {
      exec(
        http("Add Products ${name} - Modelo Abierto")
          .post("/products")
  ...
  val escenarioCerrado = scenario("Add Products - Modelo Cerrado")
    .feed(csvFeeder)
    .repeat(1) {
      exec(
        http("Add Products ${name} - Modelo Cerrado")
          .post("/products")
          .body(ElFileBody("bodyRequest/body.json"))
          .check(status.is(200))
          .check(bodyString.saveAs("response"))
